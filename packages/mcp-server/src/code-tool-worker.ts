...
import { Vibedropper, ClientOptions } from 'vibedropper';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Vibedropper)`
    : `const run: (${functionSource.client}: Vibedropper) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.lists.subscribers.add',
    'client.lists.subscribers.list',
    'client.lists.subscribers.remove',
