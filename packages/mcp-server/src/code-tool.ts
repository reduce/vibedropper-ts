...
import { ClientOptions } from 'vibedropper';

const prompt = `Runs JavaScript code to interact with the Vibedropper API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      client_envs: JSON.stringify({
        VIBEDROPPER_API_KEY: requireValue(
          readEnv('VIBEDROPPER_API_KEY') ?? client.apiKey,
          'set VIBEDROPPER_API_KEY environment variable or provide apiKey client option',
        ),
        VIBEDROPPER_BASE_URL: readEnv('VIBEDROPPER_BASE_URL') ?? client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'vibedropper',
      code,
      intent,
      client_opts: {},
